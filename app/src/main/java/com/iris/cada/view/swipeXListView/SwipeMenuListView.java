package com.iris.cada.view.swipeXListView;
...
/**
 * 在使用listview的OnItemClick的时候因为header的原因,实际使用的position要减去1,因为footer的原因,
 * 要判断点击的位置是否大于data的长度,而对于滑动删除的是正常的.
 * 
 * @author CK
 *
 */
public class SwipeMenuListView extends ListView implements OnScrollListener {
	...
	protected Scroller mScroller; // 用于回滚
	protected OnScrollListener mScrollListener; // 回滚监听
	// 触发刷新和加载更多接口.
	...
	protected int mHeaderViewHeight; // 头部View的高
	protected boolean mEnablePullRefresh = true;
	protected boolean mPullRefreshing = false; // 是否刷新.
	// -- 底部的View
	protected XListViewFooter mFooterView;
	protected boolean mEnablePullLoad = true;
	...
	protected final static int SCROLLBACK_HEADER = 0;// 顶部
	protected final static int SCROLLBACK_FOOTER = 1;// 下部

	protected final static int SCROLL_DURATION = 400; // 滚动回时间
	protected final static int PULL_LOAD_MORE_DELTA = 50; // 当大于50PX的时候，加载更多
	...
	protected SwipeMenuCreator mMenuCreator;
	protected OnMenuItemClickListener mOnMenuItemClickListener;
	protected Interpolator mCloseInterpolator;// 动画差值器
	...
	public void setAdapter(ListAdapter adapter) {
		...
		super.setAdapter(new SwipeMenuAdapter(getContext(), adapter) {
			...
			public void onItemClick(SwipeMenuView view, SwipeMenu menu, int index) {
				if (mOnMenuItemClickListener != null) {// 如果menu存在监听,则调用menu的事件处理
	 ...
	 * @param adapter
	 */
	protected void setPinnedAdapter(ListAdapter adapter) {
		// 确定XListViewFooter是最后底部的View, 并且只有一次
		if (mIsFooterReady == false) {
			mIsFooterReady = true;
			addFooterView(mFooterView);
