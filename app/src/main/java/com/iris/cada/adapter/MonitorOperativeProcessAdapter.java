...
public class MonitorOperativeProcessAdapter extends CommonAdapter<ProcessInfo> {

	public MonitorOperativeProcessAdapter(Context context, List<ProcessInfo> datas) {
		super(context, datas);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		ProcessInfo channelInfo = datas.get(position);
		viewHolder.setData(channelInfo, position, convertView);

		return viewHolder.getConvertView();
	}

	class ViewHolder extends BaseHolder<ProcessInfo> {
		...
		@Override
		protected void refreshView(ProcessInfo processInfo, int position, View convertView) {
			// TODO Auto-generated method stub
			String target = StringUtils.FormatString(processInfo.getGoal());
			String practical = StringUtils.FormatString(processInfo.getReality());
			...
			tvChannelAchieveProgress.setText(StringUtils.FormatString(processInfo.getCompletePro()) + "%");
