...
/**
 * 
 * @author lifengjuan 诊断筛选页面、。
 *
 */
public class FilterConsultantActivity extends Activity implements OnClickListener {
	...
	private List<IViewSCBean> list = new ArrayList<IViewSCBean>();
	...
	public void onCreate(Bundle arg0) {
		...
		if (!ProfitApplication.isConsultantMode) {// 车型
			...
			// 车型列表
			mCarModeList = new ArrayList<String>();//原有的汽车集合
			...
			moudes.clear();//汽车类型结合
			if(datas.size()==mCarModeList.size()){
            ...
            if(datas!=null&&datas.size()>0){
            	for(int i=0;i<datas.size();i++){
            		moudes.add(datas.get(i).getCarType());
            	}
            }
			initCarData();

		} else {
			tv_title.setText("筛选销售顾问");
			// 请求销售顾问数据
			 if(datas!=null&&datas.size()>0){
	            	for(int i=0;i<datas.size();i++){
	            		moudes.add(datas.get(i).getSalesConsultant());
	            	}
	            }
			getinfor();
		}
	...
	private void initCarData() {
		...
		for (int i = 0; i < mCarModeList.size(); i++) {
			ConsultantInfo consultantInfo = new ConsultantInfo();
			consultantInfo.setName(mCarModeList.get(i));
            consultantInfo.setCode(carCodelist.get(i));//车型code
			// 汉字转换成拼音
			String pinyin = mCharacterParser.getSelling(mCarModeList.get(i));
			...
			if (moudes == null || moudes.size()==0) {//如果穿戴过来的为空
				...
				if(moudes.contains(mCarModeList.get(i))){
	...
	private void initView() {
		...
		datas = (List<Check>) intent.getExtras().get("date");
	 ...
	 * @param list
	 */
	private void initData(List<IViewSCBean> list) {
		mCharacterParser = CharacterParser.getInstance();
		mPinyinComparator = new PinyinComparator();
		...
		for (int i = 0; i < list.size(); i++) {
			ConsultantInfo consultantInfo = new ConsultantInfo();
			consultantInfo.setName(list.get(i).getScName());
			consultantInfo.setCode(list.get(i).getScCode());

			// 汉字转换成拼音
			String pinyin = mCharacterParser.getSelling(list.get(i).getScName());
			String sortString = pinyin.substring(0, 1).toUpperCase();
			...
			if (moudes == null || moudes.size()==0) {
				...
				if(moudes.contains(list.get(i).getScName())){
					consultantInfo.setCheck(true);
				}else{
					consultantInfo.setCheck(false);
				}
...
//					checkedList.add(mCarList.get(i).getName());
					checkedList.add(mCarList.get(i).getCode());
				}
			}
		...
		if (null != mConsultantList && mConsultantList.size() != 0) {
			for (int i = 0; i < mConsultantList.size(); i++) {
				if (mConsultantList.get(i).isCheck()) {
					checkedList.add(mConsultantList.get(i).getCode());
					Log.e("checkdata1", mConsultantList.get(i).getCode());
				}
			}
			Intent intent = new Intent();
	...
	private void getinfor() {
		...
		progressDialog.show();
		// ProfitApplication.profitNetService.getIViewFilterConsultan(handler);
		ProfitApplication.profitNetService.getIViewFilterConsultan2(startTime, endTime, "", handler);
	...
	Handler handler = new MyHandler() {
		public void handleMessage(android.os.Message msg) {
			...
			case ProfitApplication.DATA_FAILED:
				ToastUtils.showMyToast(FilterConsultantActivity.this, getString(R.string.error_data));
				break;
			case ProfitApplication.DATA_SUC:
				dealChannelInfo(msg);
				break;
			case ProfitApplication.SERVER_FAILED:
				ToastUtils.showMyToast(FilterConsultantActivity.this, getString(R.string.error_net));
				break;
			}
		};
	};
	...
	private void dealChannelInfo(Message msg) {
		...
		try {
			...
			list = new Gson().fromJson(strMsg, new TypeToken<ArrayList<IViewSCBean>>() {
			}.getType());
