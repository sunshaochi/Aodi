...
public class AnyJkFrg extends Fragment implements OnClickListener{
...
//		return super.onCreateView(inflater, container, savedInstanceState);
		View view = inflater.inflate(R.layout.fragment_jk, container, false);	   
...
//	    wb_view=(WebView) view.findViewById(R.id.wb_view);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		buildVbview();
		url=ProfitApplication.H5_JK_YY+"?startDate="+stime+"&endDate="+etime+"&checkType="+mode+"&dealerCode="+ProfitApplication.loginBackInfo.getLicense();
		...
		case R.id.iv_yunyin:
			...
			if(!bool){
				if("售后半权限".equals(premission)){
					...
					bool=true;
					iv_yunyin.setImageResource(R.drawable.btn_yingli);//盈利
					close();
					buildVbview();
					url=ProfitApplication.H5_JK_YL+"?startDate="+stime+"&endDate="+etime+"&checkType="+mode+"&dealerCode="+ProfitApplication.loginBackInfo.getLicense();			
//					url=ProfitApplication.BAIDUH5;
					WebUtils.WebSet(url, wb_view, pb);
					wb_view.addJavascriptInterface(new JavaScriptinterface(getActivity()), "wst");
				}
				...
				bool=false;
				iv_yunyin.setImageResource(R.drawable.btn_yunying);//运营
				close();
				buildVbview();
				url=ProfitApplication.H5_JK_YY+"?startDate="+stime+"&endDate="+etime+"&checkType="+mode+"&dealerCode="+ProfitApplication.loginBackInfo.getLicense();			
//				url=ProfitApplication.BAIDUH5;
